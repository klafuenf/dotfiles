local c = require('ksp.colors')
local cfg = vim.g.ksp_config
local util = require("ksp.util")
...
function M.setup()
        ...
        if color_name:sub(1, 1) == '$' then
            ...
            if not color_name then
                vim.schedule(function()
                    vim.notify('ksp.nvim: unknown color "' .. name .. '"', vim.log.levels.ERROR, { title = "ksp.nvim" })
                end)
                return ""
            end
        end
    ...
    for group_name, group_settings in pairs(vim.g.ksp_config.highlights) do
